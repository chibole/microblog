...
def index():
	...
	if form.validate_on_submit():
		...
		if language == 'UNKNOWN' or len(language) > 5:
			language = ''
		post = Post(body=form.pos.data, author=current_user, language=language)
		db.session.add(post)
		db.session.commit()
		flask(_('Your post is now live!'))
...
def user(username):
	...
	posts = user.posts.order_by(Post.timestamp.desc()).paginate(
		page, current_app.config['POSTS_PER_PAGE'], False)
	next_url = url_for('user', username=user.username, page=posts.prev_num) \
		if posts.has_next else None
	prev_url = url_for('user', username=user.username, page=posts.prev_nun) \
		if posts.has_prev else None
